...
import { HistoricalChart } from './historicalChart.js?v=23';
import { DataManager } from './dataManager.js?v=10';
...
class App {
  constructor() {
    ...
    this.brokersList = getBrokersSync();
  ...
  async init() {
    ...
    // Sync temps réel : se déclenche automatiquement quand n8n écrit dans Firestore
    this.storage.marketDataSync.setupRealtimeSync(() => {
      console.log('[App] Prix synchronisés depuis Firestore (n8n)');
      this.renderCurrentPage(false);
    });
  ...
  populateAssetTypeAndBrokerSelects() {
    ...
    // Broker selects — chargés dynamiquement depuis Firestore/localStorage
    getBrokers().then(brokers => {
      const formOpts = { includeEmpty: 'Select Broker', includeAdd: true };
      const filterOpts = { includeAll: 'All Brokers', includeAdd: false };
      ...
      fillSelect(brokerSelect,    brokers, formOpts);
      fillSelect(cashBrokerSelect, brokers, formOpts);
      fillSelect(filterBroker,    brokers, filterOpts);

      const formSelects = [
        { el: brokerSelect,     opts: formOpts },
        { el: cashBrokerSelect, opts: formOpts },
      ];
      attachAddBrokerHandler(brokerSelect,    formSelects);
      attachAddBrokerHandler(cashBrokerSelect, formSelects);

      // Mettre à jour brokersList pour InvestmentsPage (label dans l'en-tête du chart)
      this.brokersList = brokers;
      if (this.investmentsPage) this.investmentsPage.brokersList = brokers;
    });
  }
  ...
  setupToggleListener() {
    ...
    toggleBtn.addEventListener('click', () => {
      ...
      // Hide onboarding bubble when form is opened
      const bubble = document.getElementById('add-hint-bubble');
      if (bubble && isExpanded) bubble.style.display = 'none';
  ...
  setupAchatsPageListeners() {
    ...
    const useCashWrapper = document.getElementById('use-cash-wrapper');
    ...
    if (transactionTypeSelect && useCashCheckbox && tickerInput && tickerSelect) {
      ...
      const updateVisibility = () => {
        ...
        if (isSell) {
          // MODE VENTE
          if (useCashWrapper) useCashWrapper.style.display = 'none';
          ...
          // MODE ACHAT (Défaut)
          if (useCashWrapper) useCashWrapper.style.display = 'flex';

          // Swap Select -> Input
          tickerSelect.style.display = 'none';
          tickerSelect.required = false;
          if (tickerWrapper) tickerWrapper.style.display = '';
      ...
      // LISTENER POUR TYPE D'ACTIF (Afficher champs Immo)
      const assetTypeSelect = document.getElementById('asset-type-select');
      const realEstateFields = document.getElementById('real-estate-fields');
      if (assetTypeSelect && realEstateFields) {
        assetTypeSelect.addEventListener('change', () => {
          if (assetTypeSelect.value === 'Real Estate') {
            realEstateFields.style.display = 'grid'; // ou flex / block
            document.getElementById('yield').required = true;
            document.getElementById('maturity').required = true;
            // Masquer Ticker (Optionnel, ou le renommer "Projet")
            document.getElementById('ticker').placeholder = "Ref. Projet (ex: P-001)";
          } else {
            realEstateFields.style.display = 'none';
            document.getElementById('yield').required = false;
            document.getElementById('maturity').required = false;
            document.getElementById('ticker').placeholder = "Ticker (e.g. AAPL)";
          }
        });
      }
    }
  ...
  initTickerAutocomplete() {
    ...
    const getKnownAssets = () => {
      const seen = new Set();
      const assets = [];
      this.storage.getPurchases()
        .filter(p => p.assetType !== 'Cash' && p.assetType !== 'Dividend' && p.ticker && p.name)
        .forEach(p => {
          const key = p.ticker.toUpperCase();
          if (!seen.has(key)) {
            seen.add(key);
            assets.push({ ticker: key, name: p.name, assetType: p.assetType || '', source: 'portfolio' });
          }
        });
      return assets;
    };
    ...
    const selectSuggestion = (ticker, name, quoteType) => {
      tickerInput.value = ticker;
      if (nameInput) nameInput.value = name;
      if (assetTypeSelect && quoteType) {
        const mapped = QUOTE_TYPE_MAP[quoteType] || quoteType;
        if ([...assetTypeSelect.options].some(o => o.value === mapped)) {
          assetTypeSelect.value = mapped;
          // Trigger change event so any dependent logic (e.g. real-estate fields) fires
    ...
    const renderSuggestions = (items) => {
      ...
      items.forEach(asset => {
        ...
        const typeLabel = QUOTE_TYPE_MAP[asset.assetType] || asset.assetType || '';
        const badge = asset.source === 'portfolio'
          ? `<span class="ticker-suggestion-badge portfolio">Portefeuille</span>`
          : (typeLabel ? `<span class="ticker-suggestion-badge">${typeLabel}</span>` : '');
        item.innerHTML =
          `<span class="ticker-suggestion-ticker">${asset.ticker}</span>` +
          `<span class="ticker-suggestion-name">${asset.name}</span>` +
          badge;
        item.addEventListener('mousedown', (e) => {
          e.preventDefault();
          selectSuggestion(asset.ticker, asset.name, asset.assetType);
        });
    ...
    const getLocalMatches = (q) => {
      const upper = q.trim().toUpperCase();
      return getKnownAssets().filter(a =>
        a.ticker.includes(upper) || a.name.toUpperCase().includes(upper)
      );
    };

    const fetchApiSuggestions = async (query) => {
      try {
        const url = `${PRICE_PROXY_URL}?symbol=${encodeURIComponent(query)}&type=SEARCH`;
        ...
        const knownTickers = new Set(getKnownAssets().map(a => a.ticker));
        const apiItems = quotes
          .filter(q => !knownTickers.has(q.symbol))
          .map(q => ({
            ticker: q.symbol,
        ...
        // Current input may have changed — re-fetch local matches
        const currentQ = tickerInput.value.trim();
        renderSuggestions([...getLocalMatches(currentQ), ...apiItems]);
      } catch {
        // Local suggestions already shown — silent fail
      }
    };
    ...
    tickerInput.addEventListener('input', e => {
      ...
      renderSuggestions(getLocalMatches(q));
      // API search after debounce
      if (searchDebounce) clearTimeout(searchDebounce);
      if (q.length >= 2) {
        searchDebounce = setTimeout(() => fetchApiSuggestions(q), 280);
      } else if (!q) {
        hideSuggestions();
      }
    });
    ...
    tickerInput.addEventListener('focus', e => {
      if (e.target.value.trim()) renderSuggestions(getLocalMatches(e.target.value));
    });
    tickerInput.addEventListener('blur', () => setTimeout(hideSuggestions, 150));

    tickerInput.addEventListener('keydown', e => {
      const items = suggestionsBox.querySelectorAll('.ticker-suggestion-item');
  ...
  async addPurchase() {
    try {
      ...
      // CRITICAL FIX: Fetch price immediately for the new asset so it displays directly
      const assetTypeLower = (purchase.assetType || 'Stock').toLowerCase();
      if (assetTypeLower !== 'cash' && assetTypeLower !== 'dividend' && assetTypeLower !== 'real estate') {
        const fetchBtn = document.getElementById('submit-btn') || { disabled: false, textContent: 'Add' };
        const prevText = fetchBtn.textContent;
        fetchBtn.disabled = true;
        fetchBtn.textContent = 'Fetching...';
        await this.api.fetchBatchPrices([purchase.ticker.toUpperCase()], true); // Force fetch
