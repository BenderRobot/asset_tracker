...
export class FilterManager {
  ...
  updateTickerFilter(onFilterChange) {
    ...
    // Build net quantity per ticker to exclude fully-sold positions
    const netQty = new Map();
    const nameMap = new Map();
    this.storage.getPurchases().forEach(p => {
      const type = (p.assetType || 'Stock').toLowerCase();
      if (type === 'cash' || type === 'dividend' || p.type === 'dividend') return;
      const t = p.ticker.toUpperCase();
      netQty.set(t, (netQty.get(t) || 0) + (parseFloat(p.quantity) || 0));
